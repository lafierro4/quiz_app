...
Future<Quiz> makeCurrentQuiz(String username, String pin) async {
  ...
  int numberQuestions =
      Random().nextInt(11) + 5; //min # of questions 5 & max # of question 15
...
Future<Image> _imageGet(String imageUrl) async {
  ...
  return Image.network(
      "https://www.cs.utep.edu/cheon/cs4381/homework/quiz/figure.php?name=$imageUrl");
}
...
_getQuestionPool(String username, String pin) async {
  ...
  for (int i = 1; i <= 15; i++) {
    ...
    var url =
        Uri.https("www.cs.utep.edu", "/cheon/cs4381/homework/quiz/get.php/", {
      "user": username,
      "pin": pin,
      "quiz": cQuiz,
    });
        ...
        children: <Widget>[
          ...
          if (widget.question.figure != null) ...[
            SizedBox(
                height: 200,
                width: 100,
                child: _loadImage(widget.question.figure as String)),
          ],
          if (widget.question.type == 1)
            _ShowOptions(
              question: widget.question as MultipleChoice,
            ),
...
class _AnswerFieldW extends State<_AnswerField> {
  final GlobalKey<FormFieldState<String>> userResponse = GlobalKey();
  ...
  Widget build(BuildContext context) {
    return Column(children: <Widget>[
      for (int i = 1; i <= widget.question.options.length; i++) ...[
        Row(children: <Widget>[
          Radio<int>(
            value: widget.question.options
                    .indexOf(widget.question.options[i - 1]) +
                1,
...
class QuizStart extends StatefulWidget {
  const QuizStart({super.key, required this.quiz, required this.currentQuestion});
...
class _QuizStart extends State<QuizStart> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        backgroundColor: Colors.black,
        appBar: AppBar(
          title: Text("Question ${widget.currentQuestion + 1}"),
        ...
        body: Center(
          child: Column(children: <Widget>[
            Expanded(
                flex: 2,
                child: AskQuestion(
                  question: widget.quiz.question[widget.currentQuestion],
                )),
            BottomNavigationBar(
                    ...
                    setState(() {
                      if(widget.currentQuestion != 0) {
                        Navigator.of(context).pushReplacementNamed('/quizStart',
                          arguments: QuizArgs.inQuiz(widget.quiz, widget.currentQuestion-1));
                      }
                    });
                  } else if (_currentIndex == 1) {
                    setState(() {
                      if(widget.currentQuestion+1 >= widget.quiz.question.length){
                        Navigator.of(context).pushReplacementNamed('/gradeQuiz',
                            arguments: QuizArgs.grade(widget.quiz));
                      }else {
                        Navigator.of(context).pushReplacementNamed('/quizStart',
                            arguments: QuizArgs.inQuiz(widget.quiz, widget.currentQuestion+1));
                      }
  ...
  gradeQuiz(Quiz quiz) {
    ...
    for (Question q in quiz.question) {
      if (q.type == 1) {
        ...
        if ((q as FillInBlank).response != null) {
          for(int i = 0; i< q.answer.length; i++) {
            if(equalsIgnoreCase(q.response!, q.answer[i])) {
              finalGrade += worth;
              numRight++;
            }
          }
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: <Widget>[
            textFormatted("You Have Completed the Quiz",),
            textFormatted("You got $numRight out of ${widget.quiz.question.length} questions right"),
            textFormatted("Your Final Score is ${finalGrade.toStringAsFixed(2)}"),
            textFormatted("Have a Good Day"),
          ],
        ),
      ),
    );
